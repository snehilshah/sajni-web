 ...
 *  <M3CookieLoader />      — shape-morphing first-paint loader.
...
export function M3Shapes() {
  return (
    <svg width="0" height="0" style={{ position: 'absolute' }} aria-hidden="true">
      <defs>
        {/* Cookie — 8-lobed blob (M3 expressive's signature shape) */}
...
export function M3CookieLoader({
  ...
  return (
    <span
      role="status"
      aria-label="Loading"
      className={`m3-cookie m3-cookie-${size} ${tone !== 'primary' ? tone : ''} ${className || ''}`.trim()}
    />
  );
}
